...
def plotRGB(sorted_data, ppm_values_str):
    ...
    #plot the RGB_Mean Intensity of the paper sensor that was taken
    plt.plot(sorted_data[:,0].astype(float),sorted_data[:,2].astype(float),color='red', marker='o', linestyle='dashed', label = "Red")
    plt.plot(sorted_data[:,0].astype(float),sorted_data[:,3].astype(float),color='green', marker='o', linestyle='dashed', label = 'Green')
    plt.plot(sorted_data[:,0].astype(float),sorted_data[:,4].astype(float),color='blue', marker='o', linestyle='dashed', label= 'Blue')
    for a,b in zip(sorted_data[:,0].astype(float).astype(int), sorted_data[:,2].astype(float).astype(int)): 
        plt.text(a+1, b+1, str(b))
