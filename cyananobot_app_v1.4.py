# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'gui1.ui'
#
# Created by: PyQt5 UI code generator 5.15.4
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets
from PyQt5.QtCore import QProcess

class Ui_CYANanoBotApp(object):
    def setupUi(self, CYANanoBotApp):
        CYANanoBotApp.setObjectName("CYANanoBotApp")
        CYANanoBotApp.setEnabled(True)
        CYANanoBotApp.resize(1106, 885)
        CYANanoBotApp.setTabShape(QtWidgets.QTabWidget.Rounded)
        self.centralwidget = QtWidgets.QWidget(CYANanoBotApp)
        self.centralwidget.setObjectName("centralwidget")
        self.label = QtWidgets.QLabel(self.centralwidget)
        self.label.setGeometry(QtCore.QRect(390, 50, 411, 51))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(20)
        font.setBold(True)
        font.setWeight(75)
        self.label.setFont(font)
        self.label.setLayoutDirection(QtCore.Qt.LeftToRight)
        self.label.setAutoFillBackground(False)
        self.label.setObjectName("label")
        self.groupBox = QtWidgets.QGroupBox(self.centralwidget)
        self.groupBox.setGeometry(QtCore.QRect(40, 100, 1041, 121))
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.groupBox.setFont(font)
        self.groupBox.setAutoFillBackground(True)
        self.groupBox.setFlat(False)
        self.groupBox.setCheckable(False)
        self.groupBox.setObjectName("groupBox")
        self.iap_button = QtWidgets.QPushButton(self.groupBox)
        self.iap_button.setGeometry(QtCore.QRect(0, 30, 260, 80))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.iap_button.setFont(font)
        self.iap_button.setObjectName("iap_button")
        self.cep_button = QtWidgets.QPushButton(self.groupBox)
        self.cep_button.setEnabled(True)
        self.cep_button.setGeometry(QtCore.QRect(260, 30, 260, 80))
        self.cep_button.setMaximumSize(QtCore.QSize(16777215, 16777215))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.cep_button.setFont(font)
        self.cep_button.setObjectName("cep_button")
        self.rap_button = QtWidgets.QPushButton(self.groupBox)
        self.rap_button.setGeometry(QtCore.QRect(520, 30, 260, 80))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.rap_button.setFont(font)
        self.rap_button.setObjectName("rap_button")
        self.pdp_button = QtWidgets.QPushButton(self.groupBox)
        self.pdp_button.setGeometry(QtCore.QRect(780, 30, 260, 80))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.pdp_button.setFont(font)
        self.pdp_button.setObjectName("pdp_button")
        self.label_3 = QtWidgets.QLabel(self.centralwidget)
        self.label_3.setGeometry(QtCore.QRect(100, 620, 91, 21))
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.label_3.setFont(font)
        self.label_3.setObjectName("label_3")
        self.cmd_line_run = QtWidgets.QPlainTextEdit(self.centralwidget)
        self.cmd_line_run.setGeometry(QtCore.QRect(100, 640, 841, 31))
        font = QtGui.QFont()
        font.setPointSize(10)
        self.cmd_line_run.setFont(font)
        self.cmd_line_run.setMouseTracking(True)
        self.cmd_line_run.setPlainText("")
        self.cmd_line_run.setObjectName("cmd_line_run")
        self.groupBox_2 = QtWidgets.QGroupBox(self.centralwidget)
        self.groupBox_2.setGeometry(QtCore.QRect(300, 230, 521, 311))
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.groupBox_2.setFont(font)
        self.groupBox_2.setObjectName("groupBox_2")
        self.layoutWidget = QtWidgets.QWidget(self.groupBox_2)
        self.layoutWidget.setGeometry(QtCore.QRect(10, 30, 501, 263))
        self.layoutWidget.setObjectName("layoutWidget")
        self.verticalLayout_3 = QtWidgets.QVBoxLayout(self.layoutWidget)
        self.verticalLayout_3.setContentsMargins(0, 0, 0, 0)
        self.verticalLayout_3.setObjectName("verticalLayout_3")
        self.formLayout = QtWidgets.QFormLayout()
        self.formLayout.setObjectName("formLayout")
        self.label_2 = QtWidgets.QLabel(self.layoutWidget)
        self.label_2.setObjectName("label_2")
        self.formLayout.setWidget(0, QtWidgets.QFormLayout.LabelRole, self.label_2)
        self.ia_paper_sensor = QtWidgets.QLineEdit(self.layoutWidget)
        self.ia_paper_sensor.setObjectName("ia_paper_sensor")
        self.formLayout.setWidget(0, QtWidgets.QFormLayout.FieldRole, self.ia_paper_sensor)
        self.label_4 = QtWidgets.QLabel(self.layoutWidget)
        self.label_4.setObjectName("label_4")
        self.formLayout.setWidget(1, QtWidgets.QFormLayout.LabelRole, self.label_4)
        self.ia_cn_concentration = QtWidgets.QLineEdit(self.layoutWidget)
        self.ia_cn_concentration.setObjectName("ia_cn_concentration")
        self.formLayout.setWidget(1, QtWidgets.QFormLayout.FieldRole, self.ia_cn_concentration)
        self.label_5 = QtWidgets.QLabel(self.layoutWidget)
        self.label_5.setObjectName("label_5")
        self.formLayout.setWidget(2, QtWidgets.QFormLayout.LabelRole, self.label_5)
        self.ia_exposure = QtWidgets.QLineEdit(self.layoutWidget)
        self.ia_exposure.setObjectName("ia_exposure")
        self.formLayout.setWidget(2, QtWidgets.QFormLayout.FieldRole, self.ia_exposure)
        self.label_8 = QtWidgets.QLabel(self.layoutWidget)
        self.label_8.setObjectName("label_8")
        self.formLayout.setWidget(3, QtWidgets.QFormLayout.LabelRole, self.label_8)
        self.ia_directory = QtWidgets.QLineEdit(self.layoutWidget)
        self.ia_directory.setObjectName("ia_directory")
        self.formLayout.setWidget(3, QtWidgets.QFormLayout.FieldRole, self.ia_directory)
        self.label_6 = QtWidgets.QLabel(self.layoutWidget)
        self.label_6.setObjectName("label_6")
        self.formLayout.setWidget(4, QtWidgets.QFormLayout.LabelRole, self.label_6)
        self.ia_resolution_combo = QtWidgets.QComboBox(self.layoutWidget)
        self.ia_resolution_combo.setObjectName("ia_resolution_combo")
        self.ia_resolution_combo.addItem("")
        self.ia_resolution_combo.addItem("")
        self.ia_resolution_combo.addItem("")
        self.formLayout.setWidget(4, QtWidgets.QFormLayout.FieldRole, self.ia_resolution_combo)
        self.label_9 = QtWidgets.QLabel(self.layoutWidget)
        self.label_9.setObjectName("label_9")
        self.formLayout.setWidget(5, QtWidgets.QFormLayout.LabelRole, self.label_9)
        self.label_7 = QtWidgets.QLabel(self.layoutWidget)
        self.label_7.setObjectName("label_7")
        self.formLayout.setWidget(6, QtWidgets.QFormLayout.LabelRole, self.label_7)
        self.ia_num_images_combo = QtWidgets.QComboBox(self.layoutWidget)
        self.ia_num_images_combo.setObjectName("ia_num_images_combo")
        self.ia_num_images_combo.addItem("")
        self.ia_num_images_combo.addItem("")
        self.ia_num_images_combo.addItem("")
        self.ia_num_images_combo.addItem("")
        self.ia_num_images_combo.addItem("")
        self.ia_num_images_combo.addItem("")
        self.ia_num_images_combo.addItem("")
        self.ia_num_images_combo.addItem("")
        self.ia_num_images_combo.addItem("")
        self.ia_num_images_combo.addItem("")
        self.ia_num_images_combo.addItem("")
        self.formLayout.setWidget(5, QtWidgets.QFormLayout.FieldRole, self.ia_num_images_combo)
        self.ia_time_int_combo = QtWidgets.QComboBox(self.layoutWidget)
        self.ia_time_int_combo.setObjectName("ia_time_int_combo")
        self.ia_time_int_combo.addItem("")
        self.ia_time_int_combo.addItem("")
        self.ia_time_int_combo.addItem("")
        self.ia_time_int_combo.addItem("")
        self.ia_time_int_combo.addItem("")
        self.ia_time_int_combo.addItem("")
        self.ia_time_int_combo.addItem("")
        self.ia_time_int_combo.addItem("")
        self.ia_time_int_combo.addItem("")
        self.ia_time_int_combo.addItem("")
        self.ia_time_int_combo.addItem("")
        self.ia_time_int_combo.addItem("")
        self.formLayout.setWidget(6, QtWidgets.QFormLayout.FieldRole, self.ia_time_int_combo)
        self.verticalLayout_3.addLayout(self.formLayout)
        self.horizontalLayout_6 = QtWidgets.QHBoxLayout()
        self.horizontalLayout_6.setObjectName("horizontalLayout_6")
        self.ia_clear_all_button = QtWidgets.QPushButton(self.layoutWidget)
        self.ia_clear_all_button.setObjectName("ia_clear_all_button")
        self.horizontalLayout_6.addWidget(self.ia_clear_all_button)
        self.ia_apply_button = QtWidgets.QPushButton(self.layoutWidget)
        self.ia_apply_button.setObjectName("ia_apply_button")
        self.horizontalLayout_6.addWidget(self.ia_apply_button)
        self.verticalLayout_3.addLayout(self.horizontalLayout_6)
        self.ia_run_button = QtWidgets.QPushButton(self.layoutWidget)
        font = QtGui.QFont()
        font.setPointSize(14)
        font.setBold(True)
        font.setWeight(75)
        self.ia_run_button.setFont(font)
        self.ia_run_button.setObjectName("ia_run_button")
        self.verticalLayout_3.addWidget(self.ia_run_button)
        self.groupBox_5 = QtWidgets.QGroupBox(self.centralwidget)
        self.groupBox_5.setEnabled(True)
        self.groupBox_5.setGeometry(QtCore.QRect(300, 230, 521, 301))
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.groupBox_5.setFont(font)
        self.groupBox_5.setObjectName("groupBox_5")
        self.layoutWidget1 = QtWidgets.QWidget(self.groupBox_5)
        self.layoutWidget1.setGeometry(QtCore.QRect(10, 30, 501, 257))
        self.layoutWidget1.setObjectName("layoutWidget1")
        self.verticalLayout_4 = QtWidgets.QVBoxLayout(self.layoutWidget1)
        self.verticalLayout_4.setContentsMargins(0, 0, 0, 0)
        self.verticalLayout_4.setObjectName("verticalLayout_4")
        self.formLayout_7 = QtWidgets.QFormLayout()
        self.formLayout_7.setObjectName("formLayout_7")
        self.label_35 = QtWidgets.QLabel(self.layoutWidget1)
        self.label_35.setObjectName("label_35")
        self.formLayout_7.setWidget(0, QtWidgets.QFormLayout.LabelRole, self.label_35)
        self.ra_paper_sensor = QtWidgets.QLineEdit(self.layoutWidget1)
        self.ra_paper_sensor.setObjectName("ra_paper_sensor")
        self.formLayout_7.setWidget(0, QtWidgets.QFormLayout.FieldRole, self.ra_paper_sensor)
        self.label_36 = QtWidgets.QLabel(self.layoutWidget1)
        self.label_36.setObjectName("label_36")
        self.formLayout_7.setWidget(1, QtWidgets.QFormLayout.LabelRole, self.label_36)
        self.ra_id = QtWidgets.QLineEdit(self.layoutWidget1)
        self.ra_id.setObjectName("ra_id")
        self.formLayout_7.setWidget(1, QtWidgets.QFormLayout.FieldRole, self.ra_id)
        self.label_11 = QtWidgets.QLabel(self.layoutWidget1)
        self.label_11.setObjectName("label_11")
        self.formLayout_7.setWidget(2, QtWidgets.QFormLayout.LabelRole, self.label_11)
        self.ra_features = QtWidgets.QComboBox(self.layoutWidget1)
        self.ra_features.setEditable(True)
        self.ra_features.setObjectName("ra_features")
        self.ra_features.addItem("")
        self.ra_features.addItem("")
        self.ra_features.addItem("")
        self.ra_features.addItem("")
        self.ra_features.addItem("")
        self.ra_features.addItem("")
        self.ra_features.addItem("")
        self.ra_features.addItem("")
        self.ra_features.addItem("")
        self.ra_features.addItem("")
        self.ra_features.addItem("")
        self.ra_features.addItem("")
        self.ra_features.addItem("")
        self.ra_features.addItem("")
        self.ra_features.addItem("")
        self.formLayout_7.setWidget(2, QtWidgets.QFormLayout.FieldRole, self.ra_features)
        self.verticalLayout_4.addLayout(self.formLayout_7)
        self.verticalLayout = QtWidgets.QVBoxLayout()
        self.verticalLayout.setObjectName("verticalLayout")
        self.ra_lr_radioButton = QtWidgets.QRadioButton(self.layoutWidget1)
        self.ra_lr_radioButton.setObjectName("ra_lr_radioButton")
        self.verticalLayout.addWidget(self.ra_lr_radioButton)
        self.ra_mlr_radioButton = QtWidgets.QRadioButton(self.layoutWidget1)
        self.ra_mlr_radioButton.setObjectName("ra_mlr_radioButton")
        self.verticalLayout.addWidget(self.ra_mlr_radioButton)
        self.ra_pr_radioButton = QtWidgets.QRadioButton(self.layoutWidget1)
        self.ra_pr_radioButton.setObjectName("ra_pr_radioButton")
        self.verticalLayout.addWidget(self.ra_pr_radioButton)
        self.ra_mpr_radioButton = QtWidgets.QRadioButton(self.layoutWidget1)
        self.ra_mpr_radioButton.setObjectName("ra_mpr_radioButton")
        self.verticalLayout.addWidget(self.ra_mpr_radioButton)
        self.verticalLayout_4.addLayout(self.verticalLayout)
        self.horizontalLayout_5 = QtWidgets.QHBoxLayout()
        self.horizontalLayout_5.setObjectName("horizontalLayout_5")
        self.ra_clear_all_button = QtWidgets.QPushButton(self.layoutWidget1)
        self.ra_clear_all_button.setObjectName("ra_clear_all_button")
        self.horizontalLayout_5.addWidget(self.ra_clear_all_button)
        self.ra_apply_button = QtWidgets.QPushButton(self.layoutWidget1)
        self.ra_apply_button.setObjectName("ra_apply_button")
        self.horizontalLayout_5.addWidget(self.ra_apply_button)
        self.verticalLayout_4.addLayout(self.horizontalLayout_5)
        self.ra_run_button = QtWidgets.QPushButton(self.layoutWidget1)
        font = QtGui.QFont()
        font.setPointSize(14)
        font.setBold(True)
        font.setWeight(75)
        self.ra_run_button.setFont(font)
        self.ra_run_button.setObjectName("ra_run_button")
        self.verticalLayout_4.addWidget(self.ra_run_button)
        self.terminal = QtWidgets.QTextBrowser(self.centralwidget)
        self.terminal.setGeometry(QtCore.QRect(100, 710, 841, 101))
        self.terminal.setObjectName("terminal")
        self.enter_button = QtWidgets.QPushButton(self.centralwidget)
        self.enter_button.setGeometry(QtCore.QRect(950, 710, 81, 51))
        font = QtGui.QFont()
        font.setPointSize(14)
        self.enter_button.setFont(font)
        self.enter_button.setObjectName("enter_button")
        self.label_12 = QtWidgets.QLabel(self.centralwidget)
        self.label_12.setGeometry(QtCore.QRect(100, 680, 91, 41))
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.label_12.setFont(font)
        self.label_12.setObjectName("label_12")
        self.clear_button = QtWidgets.QPushButton(self.centralwidget)
        self.clear_button.setGeometry(QtCore.QRect(950, 760, 81, 51))
        font = QtGui.QFont()
        font.setPointSize(14)
        self.clear_button.setFont(font)
        self.clear_button.setObjectName("clear_button")
        self.label_13 = QtWidgets.QLabel(self.centralwidget)
        self.label_13.setGeometry(QtCore.QRect(1000, 0, 101, 131))
        self.label_13.setText("")
        self.label_13.setPixmap(QtGui.QPixmap("Caraga_State_University_logo.png"))
        self.label_13.setScaledContents(True)
        self.label_13.setObjectName("label_13")
        self.label_14 = QtWidgets.QLabel(self.centralwidget)
        self.label_14.setGeometry(QtCore.QRect(50, 10, 181, 71))
        self.label_14.setText("")
        self.label_14.setPixmap(QtGui.QPixmap("Cyananobot_logo.PNG"))
        self.label_14.setScaledContents(True)
        self.label_14.setObjectName("label_14")
        self.label_16 = QtWidgets.QLabel(self.centralwidget)
        self.label_16.setGeometry(QtCore.QRect(700, 800, 301, 41))
        self.label_16.setObjectName("label_16")
        self.groupBox_3 = QtWidgets.QGroupBox(self.centralwidget)
        self.groupBox_3.setGeometry(QtCore.QRect(300, 230, 521, 221))
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.groupBox_3.setFont(font)
        self.groupBox_3.setObjectName("groupBox_3")
        self.layoutWidget2 = QtWidgets.QWidget(self.groupBox_3)
        self.layoutWidget2.setGeometry(QtCore.QRect(10, 30, 501, 177))
        self.layoutWidget2.setObjectName("layoutWidget2")
        self.verticalLayout_5 = QtWidgets.QVBoxLayout(self.layoutWidget2)
        self.verticalLayout_5.setContentsMargins(0, 0, 0, 0)
        self.verticalLayout_5.setObjectName("verticalLayout_5")
        self.formLayout_3 = QtWidgets.QFormLayout()
        self.formLayout_3.setObjectName("formLayout_3")
        self.label_15 = QtWidgets.QLabel(self.layoutWidget2)
        self.label_15.setObjectName("label_15")
        self.formLayout_3.setWidget(0, QtWidgets.QFormLayout.LabelRole, self.label_15)
        self.pd_paper_sensor = QtWidgets.QLineEdit(self.layoutWidget2)
        self.pd_paper_sensor.setObjectName("pd_paper_sensor")
        self.formLayout_3.setWidget(0, QtWidgets.QFormLayout.FieldRole, self.pd_paper_sensor)
        self.label_19 = QtWidgets.QLabel(self.layoutWidget2)
        self.label_19.setObjectName("label_19")
        self.formLayout_3.setWidget(1, QtWidgets.QFormLayout.LabelRole, self.label_19)
        self.pd_directory = QtWidgets.QLineEdit(self.layoutWidget2)
        self.pd_directory.setObjectName("pd_directory")
        self.formLayout_3.setWidget(1, QtWidgets.QFormLayout.FieldRole, self.pd_directory)
        self.label_21 = QtWidgets.QLabel(self.layoutWidget2)
        self.label_21.setObjectName("label_21")
        self.formLayout_3.setWidget(2, QtWidgets.QFormLayout.LabelRole, self.label_21)
        self.pd_num_rows = QtWidgets.QLineEdit(self.layoutWidget2)
        self.pd_num_rows.setObjectName("pd_num_rows")
        self.formLayout_3.setWidget(2, QtWidgets.QFormLayout.FieldRole, self.pd_num_rows)
        self.pd_show_images_radButton = QtWidgets.QRadioButton(self.layoutWidget2)
        self.pd_show_images_radButton.setObjectName("pd_show_images_radButton")
        self.formLayout_3.setWidget(3, QtWidgets.QFormLayout.FieldRole, self.pd_show_images_radButton)
        self.label_10 = QtWidgets.QLabel(self.layoutWidget2)
        self.label_10.setObjectName("label_10")
        self.formLayout_3.setWidget(3, QtWidgets.QFormLayout.LabelRole, self.label_10)
        self.verticalLayout_5.addLayout(self.formLayout_3)
        self.horizontalLayout = QtWidgets.QHBoxLayout()
        self.horizontalLayout.setObjectName("horizontalLayout")
        self.pd_clear_all_button = QtWidgets.QPushButton(self.layoutWidget2)
        self.pd_clear_all_button.setObjectName("pd_clear_all_button")
        self.horizontalLayout.addWidget(self.pd_clear_all_button)
        self.pd_apply_button = QtWidgets.QPushButton(self.layoutWidget2)
        self.pd_apply_button.setObjectName("pd_apply_button")
        self.horizontalLayout.addWidget(self.pd_apply_button)
        self.verticalLayout_5.addLayout(self.horizontalLayout)
        self.pd_run_button = QtWidgets.QPushButton(self.layoutWidget2)
        font = QtGui.QFont()
        font.setPointSize(14)
        font.setBold(True)
        font.setWeight(75)
        self.pd_run_button.setFont(font)
        self.pd_run_button.setObjectName("pd_run_button")
        self.verticalLayout_5.addWidget(self.pd_run_button)
        self.groupBox_4 = QtWidgets.QGroupBox(self.centralwidget)
        self.groupBox_4.setGeometry(QtCore.QRect(300, 230, 521, 211))
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.groupBox_4.setFont(font)
        self.groupBox_4.setObjectName("groupBox_4")
        self.layoutWidget3 = QtWidgets.QWidget(self.groupBox_4)
        self.layoutWidget3.setGeometry(QtCore.QRect(10, 30, 501, 171))
        self.layoutWidget3.setObjectName("layoutWidget3")
        self.verticalLayout_2 = QtWidgets.QVBoxLayout(self.layoutWidget3)
        self.verticalLayout_2.setContentsMargins(0, 0, 0, 0)
        self.verticalLayout_2.setObjectName("verticalLayout_2")
        self.formLayout_5 = QtWidgets.QFormLayout()
        self.formLayout_5.setObjectName("formLayout_5")
        self.label_29 = QtWidgets.QLabel(self.layoutWidget3)
        self.label_29.setObjectName("label_29")
        self.formLayout_5.setWidget(0, QtWidgets.QFormLayout.LabelRole, self.label_29)
        self.ce_paper_sensor = QtWidgets.QLineEdit(self.layoutWidget3)
        self.ce_paper_sensor.setObjectName("ce_paper_sensor")
        self.formLayout_5.setWidget(0, QtWidgets.QFormLayout.FieldRole, self.ce_paper_sensor)
        self.label_30 = QtWidgets.QLabel(self.layoutWidget3)
        self.label_30.setObjectName("label_30")
        self.formLayout_5.setWidget(1, QtWidgets.QFormLayout.LabelRole, self.label_30)
        self.label_32 = QtWidgets.QLabel(self.layoutWidget3)
        self.label_32.setObjectName("label_32")
        self.formLayout_5.setWidget(2, QtWidgets.QFormLayout.LabelRole, self.label_32)
        self.ce_rf2 = QtWidgets.QLineEdit(self.layoutWidget3)
        self.ce_rf2.setObjectName("ce_rf2")
        self.formLayout_5.setWidget(2, QtWidgets.QFormLayout.FieldRole, self.ce_rf2)
        self.ce_id = QtWidgets.QLineEdit(self.layoutWidget3)
        self.ce_id.setObjectName("ce_id")
        self.formLayout_5.setWidget(1, QtWidgets.QFormLayout.FieldRole, self.ce_id)
        self.verticalLayout_2.addLayout(self.formLayout_5)
        self.horizontalLayout_3 = QtWidgets.QHBoxLayout()
        self.horizontalLayout_3.setObjectName("horizontalLayout_3")
        self.ce_clear_all_button = QtWidgets.QPushButton(self.layoutWidget3)
        self.ce_clear_all_button.setObjectName("ce_clear_all_button")
        self.horizontalLayout_3.addWidget(self.ce_clear_all_button)
        self.ce_apply_button = QtWidgets.QPushButton(self.layoutWidget3)
        self.ce_apply_button.setObjectName("ce_apply_button")
        self.horizontalLayout_3.addWidget(self.ce_apply_button)
        self.verticalLayout_2.addLayout(self.horizontalLayout_3)
        self.ce_run_button = QtWidgets.QPushButton(self.layoutWidget3)
        font = QtGui.QFont()
        font.setPointSize(14)
        font.setBold(True)
        font.setWeight(75)
        self.ce_run_button.setFont(font)
        self.ce_run_button.setObjectName("ce_run_button")
        self.verticalLayout_2.addWidget(self.ce_run_button)
        CYANanoBotApp.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(CYANanoBotApp)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 1106, 21))
        self.menubar.setObjectName("menubar")
        CYANanoBotApp.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(CYANanoBotApp)
        self.statusbar.setObjectName("statusbar")
        CYANanoBotApp.setStatusBar(self.statusbar)

        self.retranslateUi(CYANanoBotApp)
        QtCore.QMetaObject.connectSlotsByName(CYANanoBotApp)

        self.p = None
        self.pd_apply_button.clicked.connect(self.roi_extraction)
        self.ce_apply_button.clicked.connect(self.color_extraction)
        self.ra_apply_button.clicked.connect(self.regression_analysis)
        self.ia_apply_button.clicked.connect(self.image_acquisition)

        

        self.ra_clear_all_button.clicked.connect(self.clear_fields_ra)
        self.ce_clear_all_button.clicked.connect(self.clear_fields_ce)
        self.pd_clear_all_button.clicked.connect(self.clear_fields_pd)


        self.iap_button.clicked.connect(self.iap_button_clicked)
        self.rap_button.clicked.connect(self.rap_button_clicked)
        self.cep_button.clicked.connect(self.cep_button_clicked)
        self.pdp_button.clicked.connect(self.pdp_button_clicked)


        self.groupBox_2.hide()
        self.groupBox_3.hide()
        self.groupBox_4.hide()
        self.groupBox_5.hide()

        self.ce_run_button.clicked.connect(self.run_cep)
        self.pd_run_button.clicked.connect(self.run_pdp)
        self.ra_run_button.clicked.connect(self.run_rap)
        self.ia_run_button.clicked.connect(self.run_iap)

        self.enter_button.clicked.connect(self.on_clicked_enter)
        self.clear_button.clicked.connect(self.on_clicked_clear)
    

    def retranslateUi(self, CYANanoBotApp):
        _translate = QtCore.QCoreApplication.translate
        CYANanoBotApp.setWindowTitle(_translate("CYANanoBotApp", "CYANanoBotApp"))
        self.label.setText(_translate("CYANanoBotApp", "CYANanoBot Software V.1.0"))
        self.groupBox.setTitle(_translate("CYANanoBotApp", "Programs"))
        self.iap_button.setText(_translate("CYANanoBotApp", "Image Acquisition Program"))
        self.cep_button.setText(_translate("CYANanoBotApp", "Color Extraction Program"))
        self.rap_button.setText(_translate("CYANanoBotApp", "Regression Analysis Program"))
        self.pdp_button.setText(_translate("CYANanoBotApp", "Paper Detection Program"))
        self.label_3.setText(_translate("CYANanoBotApp", "Command Line"))
        self.groupBox_2.setTitle(_translate("CYANanoBotApp", "Image Acquisition Program Parameters"))
        self.label_2.setText(_translate("CYANanoBotApp", "Name of Paper Sensor "))
        self.label_4.setText(_translate("CYANanoBotApp", "Cyanide Concentration"))
        self.label_5.setText(_translate("CYANanoBotApp", "Exposure"))
        self.label_8.setText(_translate("CYANanoBotApp", "Directory"))
        self.label_6.setText(_translate("CYANanoBotApp", "Resolution"))
        self.ia_resolution_combo.setItemText(0, _translate("CYANanoBotApp", "High Resolution (2592, 1952)"))
        self.ia_resolution_combo.setItemText(1, _translate("CYANanoBotApp", "Medium Resolution (1024, 768)"))
        self.ia_resolution_combo.setItemText(2, _translate("CYANanoBotApp", "Low Resolution (640,480)"))
        self.label_9.setText(_translate("CYANanoBotApp", "Number of Images"))
        self.label_7.setText(_translate("CYANanoBotApp", "Time Interval (sec)"))
        self.ia_num_images_combo.setCurrentText(_translate("CYANanoBotApp", "5"))
        self.ia_num_images_combo.setItemText(0, _translate("CYANanoBotApp", "5"))
        self.ia_num_images_combo.setItemText(1, _translate("CYANanoBotApp", "10"))
        self.ia_num_images_combo.setItemText(2, _translate("CYANanoBotApp", "20"))
        self.ia_num_images_combo.setItemText(3, _translate("CYANanoBotApp", "30"))
        self.ia_num_images_combo.setItemText(4, _translate("CYANanoBotApp", "40"))
        self.ia_num_images_combo.setItemText(5, _translate("CYANanoBotApp", "50"))
        self.ia_num_images_combo.setItemText(6, _translate("CYANanoBotApp", "60"))
        self.ia_num_images_combo.setItemText(7, _translate("CYANanoBotApp", "70"))
        self.ia_num_images_combo.setItemText(8, _translate("CYANanoBotApp", "80"))
        self.ia_num_images_combo.setItemText(9, _translate("CYANanoBotApp", "90"))
        self.ia_num_images_combo.setItemText(10, _translate("CYANanoBotApp", "100"))
        self.ia_time_int_combo.setCurrentText(_translate("CYANanoBotApp", "5"))
        self.ia_time_int_combo.setItemText(0, _translate("CYANanoBotApp", "5"))
        self.ia_time_int_combo.setItemText(1, _translate("CYANanoBotApp", "10"))
        self.ia_time_int_combo.setItemText(2, _translate("CYANanoBotApp", "15"))
        self.ia_time_int_combo.setItemText(3, _translate("CYANanoBotApp", "20"))
        self.ia_time_int_combo.setItemText(4, _translate("CYANanoBotApp", "25"))
        self.ia_time_int_combo.setItemText(5, _translate("CYANanoBotApp", "30"))
        self.ia_time_int_combo.setItemText(6, _translate("CYANanoBotApp", "35"))
        self.ia_time_int_combo.setItemText(7, _translate("CYANanoBotApp", "40"))
        self.ia_time_int_combo.setItemText(8, _translate("CYANanoBotApp", "45"))
        self.ia_time_int_combo.setItemText(9, _translate("CYANanoBotApp", "50"))
        self.ia_time_int_combo.setItemText(10, _translate("CYANanoBotApp", "55"))
        self.ia_time_int_combo.setItemText(11, _translate("CYANanoBotApp", "60"))
        self.ia_clear_all_button.setText(_translate("CYANanoBotApp", "Clear All"))
        self.ia_apply_button.setText(_translate("CYANanoBotApp", "Apply"))
        self.ia_run_button.setText(_translate("CYANanoBotApp", "Run Program"))
        self.groupBox_5.setTitle(_translate("CYANanoBotApp", "Regression Analysis Program Parameters"))
        self.label_35.setText(_translate("CYANanoBotApp", "Name of Paper Sensor "))
        self.label_36.setText(_translate("CYANanoBotApp", "images id"))
        self.label_11.setText(_translate("CYANanoBotApp", "Features"))
        self.ra_features.setItemText(0, _translate("CYANanoBotApp", "R"))
        self.ra_features.setItemText(1, _translate("CYANanoBotApp", "G"))
        self.ra_features.setItemText(2, _translate("CYANanoBotApp", "B"))
        self.ra_features.setItemText(3, _translate("CYANanoBotApp", "RGB"))
        self.ra_features.setItemText(4, _translate("CYANanoBotApp", "H"))
        self.ra_features.setItemText(5, _translate("CYANanoBotApp", "S"))
        self.ra_features.setItemText(6, _translate("CYANanoBotApp", "V"))
        self.ra_features.setItemText(7, _translate("CYANanoBotApp", "HSV"))
        self.ra_features.setItemText(8, _translate("CYANanoBotApp", "L "))
        self.ra_features.setItemText(9, _translate("CYANanoBotApp", "a"))
        self.ra_features.setItemText(10, _translate("CYANanoBotApp", "b"))
        self.ra_features.setItemText(11, _translate("CYANanoBotApp", "Lab"))
        self.ra_features.setItemText(12, _translate("CYANanoBotApp", "RGBHSV"))
        self.ra_features.setItemText(13, _translate("CYANanoBotApp", "RGBHSVLab"))
        self.ra_features.setItemText(14, _translate("CYANanoBotApp", "RGBHSVLab"))
        self.ra_lr_radioButton.setText(_translate("CYANanoBotApp", "Linear Regression"))
        self.ra_mlr_radioButton.setText(_translate("CYANanoBotApp", "Multiple Linear Regression"))
        self.ra_pr_radioButton.setText(_translate("CYANanoBotApp", "Polynomial Regressiion"))
        self.ra_mpr_radioButton.setText(_translate("CYANanoBotApp", "Multiple Polynomial Regression"))
        self.ra_clear_all_button.setText(_translate("CYANanoBotApp", "Clear All"))
        self.ra_apply_button.setText(_translate("CYANanoBotApp", "Apply"))
        self.ra_run_button.setText(_translate("CYANanoBotApp", "Run Program"))
        self.enter_button.setText(_translate("CYANanoBotApp", "Enter"))
        self.label_12.setText(_translate("CYANanoBotApp", "Terminal"))
        self.clear_button.setText(_translate("CYANanoBotApp", "Clear"))
        self.label_16.setText(_translate("CYANanoBotApp", "Developed by: Ian Benedict Pabas Pongcol"))
        self.groupBox_3.setTitle(_translate("CYANanoBotApp", "Paper Detection Program Parameters"))
        self.label_15.setText(_translate("CYANanoBotApp", "Name of Paper Sensor "))
        self.label_19.setText(_translate("CYANanoBotApp", "Directory"))
        self.label_21.setText(_translate("CYANanoBotApp", "Number of Rows"))
        self.pd_show_images_radButton.setText(_translate("CYANanoBotApp", "Show"))
        self.label_10.setText(_translate("CYANanoBotApp", "Show Images"))
        self.pd_clear_all_button.setText(_translate("CYANanoBotApp", "Clear All"))
        self.pd_apply_button.setText(_translate("CYANanoBotApp", "Apply"))
        self.pd_run_button.setText(_translate("CYANanoBotApp", "Run Program"))
        self.groupBox_4.setTitle(_translate("CYANanoBotApp", "Color Extraction Program Parameters"))
        self.label_29.setText(_translate("CYANanoBotApp", "Name of Paper Sensor "))
        self.label_30.setText(_translate("CYANanoBotApp", "images id"))
        self.label_32.setText(_translate("CYANanoBotApp", "ROI 2"))
        self.ce_clear_all_button.setText(_translate("CYANanoBotApp", "Clear All"))
        self.ce_apply_button.setText(_translate("CYANanoBotApp", "Apply"))
        self.ce_run_button.setText(_translate("CYANanoBotApp", "Run Program"))

    def iap_button_clicked(self):

        # self.groupBox_2.hide()
        self.groupBox_3.hide()
        self.groupBox_4.hide()
        self.groupBox_5.hide()
        self.groupBox_2.show()

    def cep_button_clicked(self):

        self.groupBox_2.hide()
        self.groupBox_3.hide()
        # self.groupBox_4.hide()
        self.groupBox_5.hide()
        self.groupBox_4.show()

    def pdp_button_clicked(self):
        self.groupBox_2.hide()
        # self.groupBox_3.hide()
        self.groupBox_4.hide()
        self.groupBox_5.hide()
        self.groupBox_3.show()


    def rap_button_clicked(self):
        self.groupBox_2.hide()
        self.groupBox_3.hide()
        self.groupBox_4.hide()
        self.groupBox_5.show()

        # self.groupBox_5.hide()


    def clear_fields_ra(self):
        self.ra_paper_sensor.clear()
        self.ra_id.clear()

    def clear_fields_ce(self):
        self.ce_paper_sensor.clear()
        self.ce_id.clear()
        self.ce_rf2.clear()

    def clear_fields_pd(self):
        self.pd_paper_sensor.clear()

        self.pd_directory.clear()

        self.pd_num_rows.clear()

    def clear_fields_ia(self):
        self.ia_paper_sensor.clear()
        self.ia_cn_concentration.clear()
        self.ia_directory.clear()
        self.ia_exposure.clear()



    def roi_extraction(self):
        self.cmd_line_run.clear()
        print("Paper sensor detection and cropping program")
        show_image= "False"

        if self.pd_show_images_radButton.isChecked() is True:
            show_image = "True"
        else:
            show_image = "False"    

        # show_image = "False"

        pd_cmd_list = [
                    'paper_sensor_detection_program.py',
                    '-ps',
                    self.pd_paper_sensor.text(),
                    '-cif',
                    self.pd_directory.text(),
                    '-nr',
                    self.pd_num_rows.text(),
                    '-si',
                    show_image]
        print(pd_cmd_list)
        pd_cmd_str = ' '.join(pd_cmd_list)
        print(pd_cmd_str)
        self.cmd_line_run.insertPlainText(pd_cmd_str)
        # pd_cmd_list1= self.cmd_line_run.toPlainText().split(" ")
        # print(pd_cmd_list1)
        # pd_cmd_str1= ' '.join(pd_cmd_list1)
        # print(pd_cmd_str1)
        # subprocess.run(pd_cmd_list1)
        # subprocess.run(['python', 'paper_sensor_detection_program.py', '-fn', 'new_sensor','-id2','10', '-rf1','ROI_newsensor', '-rf2', 'ROI2_newsensor', '-cif','captured_images3','-cisf', 'data_gathering_newsensor', '-nr','3', '-si','False'])


    def image_acquisition(self):
        self.cmd_line_run.clear()
        print("Paper sensor detection and cropping program")
        resolution_index = self.ia_resolution_combo.currentIndex()

        if resolution_index == 0:
            resolution = 'H'
        
        elif resolution_index == 1:
            resolution = 'M'

        elif resolution_index == 2:
            resolution = 'L'


        ia_cmd_list = [
                    'image_acquisition_program.py',
                    '-cn',
                    self.ia_cn_concentration.text(),
                    '-ps',
                    self.ia_paper_sensor.text(),
                    '-in',
                    self.ia_time_int_combo.currentText(),
                    '-r',
                    resolution,
                    '-cif',
                    self.ia_directory.text(),
                    '-e',
                    self.ia_exposure.text(),
                    '-i',
                    self.ia_num_images_combo.currentText()
                    ]
        print(ia_cmd_list)
        ia_cmd_str = ' '.join(ia_cmd_list)
        print(ia_cmd_str)
        self.cmd_line_run.insertPlainText(ia_cmd_str)





    def color_extraction(self):
        self.cmd_line_run.clear()
        print("Color Extraction Program")

        ce_cmd_list = [
                    'color_extraction_program.py',
                    '-ps',
                    self.ce_paper_sensor.text(),
                    '-id2',
                    self.ce_id.text()
                    ]
        print(ce_cmd_list)
        ce_cmd_str = ' '.join(ce_cmd_list)
        print(ce_cmd_str)
        self.cmd_line_run.insertPlainText(ce_cmd_str)
        # ce_cmd_list1= self.cmd_line_run.toPlainText().split(" ")
        # print(ce_cmd_list1)
        # ce_cmd_str1= ' '.join(ce_cmd_list1)
        # print(ce_cmd_str1)
        # subprocess.run(ce_cmd_list1)
        # subprocess.run(['python', 'paper_sensor_detection_program.py', '-fn', 'new_sensor','-id2','10', '-rf1','ROI_newsensor', '-rf2', 'ROI2_newsensor', '-cif','captured_images3','-cisf', 'data_gathering_newsensor', '-nr','3', '-si','False'])

    def regression_analysis(self):
        self.cmd_line_run.clear()
        feature = self.ra_features.currentText()
        
        print(feature)

        if self.ra_lr_radioButton.isChecked():
            regression_model= "Linear_Regression"
        
        elif self.ra_mlr_radioButton.isChecked():
            regression_model= "Multiple_Linear_Regression"

        elif self.ra_pr_radioButton.isChecked():
            regression_model= "Polynomial_Regression"

        elif self.ra_mpr_radioButton.isChecked():
            regression_model= "Multiple_Polynomial_Regression"


        print("Regression Analysis Program")
        
        ra_cmd_list = [
                    'create_regression_model.py',
                    '-ps',
                    self.ra_paper_sensor.text(),
                    '-id2',
                    self.ra_id.text(),
                    '-fr',
                    feature,
                    '-rm',
                    regression_model
                    ]
        print(ra_cmd_list)
        ra_cmd_str = ' '.join(ra_cmd_list)
        print(ra_cmd_str)
        self.cmd_line_run.insertPlainText(ra_cmd_str)
            # ra_cmd_list1= self.cmd_line_run.toPlainText().split(" ")
            # print(ra_cmd_list1)
            # ra_cmd_str1= ' '.join(ra_cmd_list1)
            # print(ra_cmd_str1)
            # subprocess.run(ra_cmd_list1)

    def run_iap(self):
        # cmd_line = self.cmd_line_run.text()
        iap_cmd_list1= self.cmd_line_run.toPlainText().split(" ")
        # print(cmd_line)
        print(iap_cmd_list1)
        # subprocess.run(iap_cmd_list1)
        if self.p is None: 
            self.p = QProcess()
            self.p.readyReadStandardOutput.connect(self.handle_stdout)
            self.p.readyReadStandardError.connect(self.handle_stderr)
            self.p.finished.connect(self.process_finished)
            self.p.start("python",iap_cmd_list1)


    def run_cep(self):
        # cmd_line = self.cmd_line_run.text()
        cep_cmd_list1= self.cmd_line_run.toPlainText().split(" ")
        # print(cmd_line)
        print(cep_cmd_list1)
        # subprocess.run(cep_cmd_list1)
        if self.p is None: 
            self.p = QProcess()
            self.p.readyReadStandardOutput.connect(self.handle_stdout)
            self.p.readyReadStandardError.connect(self.handle_stderr)
            self.p.finished.connect(self.process_finished)
            self.p.start("python",cep_cmd_list1)
        
        # pd_cmd_str1= ' '.join(pd_cmd_list1)
        # print(pd_cmd_str1)
    def run_pdp(self):
        # cmd_line = self.cmd_line_run.text()
        pdp_cmd_list1= self.cmd_line_run.toPlainText().split(" ")
        # print(cmd_line)
        print(pdp_cmd_list1)
        if self.p is None: 
        # subprocess.run(pdp_cmd_list1)
            self.p = QProcess()
            self.p.readyReadStandardOutput.connect(self.handle_stdout)
            self.p.readyReadStandardError.connect(self.handle_stderr)
            self.p.finished.connect(self.process_finished)
            self.p.start("python",pdp_cmd_list1)
            

    def run_rap(self):
        # cmd_line = self.cmd_line_run.text()
        rap_cmd_list1= self.cmd_line_run.toPlainText().split(" ")
        # print(cmd_line)
        print(rap_cmd_list1)
        # subprocess.run(rap_cmd_list1)
        if self.p is None: 
            self.p = QProcess()
            
            # self.p.readyRead.connect(self.dataReady)
            self.p.readyReadStandardOutput.connect(self.handle_stdout)
            self.p.readyReadStandardError.connect(self.handle_stderr)
            self.p.finished.connect(self.process_finished)
            self.p.start("python",rap_cmd_list1)

    def message(self, s):
        self.terminal.append(s)

    def handle_stderr(self):
        data = self.p.readAllStandardError()
        stderr = bytes(data).decode("utf8")
        self.message(stderr)

    def handle_stdout(self):
        data = self.p.readAllStandardOutput()
        stdout = bytes(data).decode("utf8")
        self.message(stdout)

    def process_finished(self):
        self.message("Process finished.")
        self.p = None

    def dataReady(self):
        print(str(self.p.readAll()))

    
    # @QtCore.pyqtSlot()
    def on_clicked_enter(self):
        text = " \n"
        self.p.write(text.encode())
        self.message("clicked\n")
        print("clicked")

    def on_clicked_clear(self):
        self.terminal.clear()


if __name__ == "__main__":
    import sys
    app = QtWidgets.QApplication(sys.argv)
    CYANanoBotApp = QtWidgets.QMainWindow()
    ui = Ui_CYANanoBotApp()
    ui.setupUi(CYANanoBotApp)
    CYANanoBotApp.show()
    sys.exit(app.exec_())
