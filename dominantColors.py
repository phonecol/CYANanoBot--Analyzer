...
class DominantColors:
    ...
    def getMeanIntensity(self, getHSV=True,getLab=True, getGray = True):
        ...
        if getGray:
            ...
            val_gray = np.reshape(gray[:,:],-1)
            # masked_lab = np.ma.masked_less(val_lab,20)
            img_mean_gray = np.mean(val_gray)
            img_std_gray = np.std(val_gray)
            img_MEAN_gray.append(img_mean_gray)
            img_STD_gray.append(img_std_gray)
